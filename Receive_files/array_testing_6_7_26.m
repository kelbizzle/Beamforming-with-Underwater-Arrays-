...
%% Spectrograms
% to normalize peak to 0 dB use this command in each channel: max_val = max(s_db, [], 'all');
% Ch1
% figure;
% imagesc(timeStamps, freqAxis/1000, ffts{1} - max(ffts{1}, [], 'all'));
% axis xy; 
% colorbar;
% clim([-60 0]);
% xlabel('Time (s)'); 
...
% saveas(gcf, sprintf('spec_%s.png', name1));
% 
% % 
% % Ch2
% figure;
% imagesc(timeStamps, freqAxis/1000, ffts{2} - max(ffts{2}, [], 'all'));
% axis xy;  
% colorbar;
% clim([-60 0]);
% xlabel('Time (s)'); 
% ylabel('Frequency (kHz)');
% 
% title('Spectrogram of Channel 2');
% saveas(gcf, sprintf('spec_%s.png', name2));
% 
% % Ch3
% figure;
% imagesc(timeStamps, freqAxis/1000, ffts{3} - max(ffts{3}, [], 'all'));
% axis xy;  
% colorbar;
% clim([-60 0]);
% xlabel('Time (s)'); 
% ylabel('Frequency (kHz)');
% 
% title('Spectrogram of Channel 3');
% saveas(gcf, sprintf('spec_%s.png', name3));
% 
% % Ch4
% figure;
% imagesc(timeStamps, freqAxis/1000, ffts{4} - max(ffts{4}, [], 'all'));
% axis xy;  
% colorbar;
% clim([-60 0]);
% xlabel('Time (s)'); 
% ylabel('Frequency (kHz)');
% 
% title('Spectrogram of Channel 4');
% saveas(gcf, sprintf('spec_%s.png', name4));
% 
% % Ch5
% figure;
% imagesc(timeStamps, freqAxis/1000, ffts{5} - max(ffts{5}, [], 'all'));
% axis xy; 
% colorbar;
% clim([-60 0]);
% xlabel('Time (s)'); 
% ylabel('Frequency (kHz)');
% 
% title('Spectrogram of Channel 5');
% saveas(gcf, sprintf('spec_%s.png', name5));
% 
% % Ch6
% figure;
% imagesc(timeStamps, freqAxis/1000, ffts{6} - max(ffts{6}, [], 'all'));
% axis xy; 
% colorbar;
% clim([-60 0]);
% xlabel('Time (s)'); 
% ylabel('Frequency (kHz)');
% 
% title('Spectrogram of Channel 6');
% saveas(gcf, sprintf('spec_%s.png', name6));
% 
% % Ch7
% figure;
% imagesc(timeStamps, freqAxis/1000, ffts{7} - max(ffts{7}, [], 'all'));
% axis xy; 
% colorbar;
% clim([-60 0]);
% xlabel('Time (s)'); 
% ylabel('Frequency (kHz)');
% 
% title('Spectrogram of Channel 7');
% saveas(gcf, sprintf('spec_%s.png', name7));
% 
% % Ch8
% figure;
% imagesc(timeStamps, freqAxis/1000, ffts{8} - max(ffts{8}, [], 'all'));
% axis xy; 
% colorbar;
% clim([-60 0]);
% xlabel('Time (s)'); 
% ylabel('Frequency (kHz)');
% 
% title('Spectrogram of Channel 8');
% saveas(gcf, sprintf('spec_%s.png', name8));
% 
% % k-omega Plot
% % matrix that stores data as time_samples vs. sensors
% % tmp_mat = [recordedAudio1, recordedAudio2, recordedAudio3, recordedAudio4, recordedAudio5, recordedAudio6, recordedAudio7, recordedAudio8];
% % 
% % step_f = fs / nt; % step between frequency samples
% % f_pos = 0 : step_f : fs-step_f; % nyquist frequency --> fs/2 ... n/2 + 1 points --> range: [0, 2pi]
% % f_all = -fs/2 : step_f : fs/2 - step_f; % necessary if using two shifts in double fft --> [-fs/2,fs/2)
% % step_k = (2 * pi) / (d * ns); % step between wave number samples
% % k = -pi/d : step_k : pi/d - step_k; % wave number axis
% % 
% % p_matrix = compute_2d_fft(tmp_mat); % calls function at top of file
% % array_max = max(p_matrix, [], 'all'); % computes the max of computed matrix
% % 
% % vis1 = 2 * pi * f_pos / c; % used for plotting visible region
% % vis2 = -1*(2 * pi * f_pos / c); 
% % 
% % figure;
% % imagesc(f_all, k, p_matrix-array_max); % subtracting max normalizes peak to 0 dB 
% % set(gca, 'YDir','normal')
% % hold on;
% % plot(f_pos, vis1, 'k--');
% % plot(f_pos, vis2, 'k-.');
% % xlabel('Frequency (Hz)')
% % xlim([0 (fs/2 - step_f)])
% % ylabel('k (wave number)')
% % colorbar()
% % title('k vs. omega plot ')
% % saveas(gcf, sprintf('k_omega_%s.png', tmstmp));
% % % end of file 